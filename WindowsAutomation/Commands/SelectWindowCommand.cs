...
namespace Mavaddat.Wasp
{
    ...
    public class SelectWindowCommand : PSCmdlet
    {
       ...
       HelpMessage = "Allows the input to be a filtered set of output from Get-Process")]
        ...
        protected override void ProcessRecord()
        {
            switch (ParameterSetName)
            {
                ...
                default:
                    ...
                    foreach (Window w in Mavaddat.Wasp.WindowFinder.GetTopLevelWindows())
                    {
                        WriteObject(w);
                    }
                    break;
        ...
        {
            Mavaddat.Wasp.WindowFinder.IncludeToolWindowsWithAppWindows = ToolWindows.ToBool();
            var windows = Mavaddat.Wasp.WindowFinder.GetTopLevelWindows();
            Mavaddat.Wasp.WindowFinder.IncludeToolWindowsWithAppWindows = false;

            foreach (string cls in Classes)
            {
                WildcardPattern wildcard = new WildcardPattern(cls, WildcardOptions.IgnoreCase | WildcardOptions.Compiled);
                foreach (Mavaddat.Wasp.Window window in windows)
                {
        ...
        {
            foreach (Mavaddat.Wasp.Window window in Mavaddat.Wasp.WindowFinder.GetTopLevelWindows())
            {
        ...
        private IEnumerable<Window> GetWindowsOfProcess(Process proc)
        {
            ...
            foreach (Window w in Mavaddat.Wasp.WindowFinder.GetProcessWindows(proc))
            {
                yield return w;
            }
        }
